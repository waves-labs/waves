...
export default async function userController(fastify: FastifyInstance) {
  fastify.post("/claim", async function (req: FastifyRequest, reply: FastifyReply) {
    ...
    if (!body.eventName || !body.synth || !body.artist) {
      reply.code(400).send({ error: "Missing synth or waveID" });
      return;
    }
    ...
    if (!waveID) {
      reply.code(400).send({ error: "Invalid waveID" });
    ...
    const schemaEncoder = new SchemaEncoder(
      "string eventName, address ticketAddrs, address wavesAddrs, uint256 waveId"
    );
